...
The model uses the $softmax$ function as the last output layers, because of this its outputs can be interpreted as a distribution probability over the labels given a data point (as expalined in the first section) and the prediction will be the label with the highest probability. This allows to see among the wrong predictions the ones that the model classified with the highest confidence (highest probability value). This revealed that there are instances of mislabeled examples in the test set. Figure \ref{fig:worst_PRED} shows the top forty wrong predictions with the highest confidence and clearly the majority of those data points are mislabeled. A similar analysis among the wrong predictions with the lowest confidence didn't shown any kind of mislabeling of the examples. It remains unclear the full extent to which the mislabeling affects the test set and possibly the training set, and consequently the evaluation and training of the model.
